...
export const ItemPost = ({ post = {}, isRow = true }) => {
    ...
    if (isRow) {
        return (
            <article className="row-start-2 col-span-3 md:col-span-1 rounded-md shadow-md flex flex-col overflow-hidden">
                {/* Imagen del post */}
                ...
                <div className="flex flex-col gap-4 p-4 w-full">
                    ...
                    <div className="flex flex-col gap-2 flex-1">
                        <Link>
                            <h3 className='text-xl lg:text-2xl font-extrabold hover:text-text-500 transition-all duration-200'>{post.title.toUpperCase()}</h3>
                        </Link>
        ...
        return (
            <article className='col-span-3 lg:flex gap-2 lg:max-h-[550px] rounded-md shadow-lg'>
                <Link className='w-full lg:w-[45%] flex-shrink-0'>
                ...
                <div className='w-full lg:w-[55%] flex flex-col justify-between p-4'>
                    <div className='mb-6 border-b border-text-500 py-3 flex items-center flex-wrap gap-4'>
                        <div className='flex-grow flex flex-col gap-2 items-start'>
                            ...
                            <Link>
                                <h2 className='text-xl lg:text-2xl font-extrabold hover:text-text-500 transition-all duration-200'>{post.title.toUpperCase()}</h2>
                            </Link>
                        </div>

                        <Link>
                            <MiniAuthor name={post.author} photo={post.photo} description={post.description} />
                        </Link>
                    </div>

                    <p className='text-base flex-1 lg:text-lg leading-7 text-balance line-clamp-5 '>
                        {post.content}
                    </p>

                    <div className='mt-4 flex items-center justify-between'>
                        <div className="text-text-500 font-bold">
                            <small className='text-primary-500'>{date}</small>
                            <div className="flex items-center gap-6 text-text-400 mt-2">
                                <article className="flex items-center gap-2">
                                    <AiFillLike />
                                    <p>{post.like_count}</p>
                                </article>
